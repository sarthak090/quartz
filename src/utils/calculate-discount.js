export const caluclateDiscount = (cartData) => {
  const { items, discountData, subtotal } = cartData;
...
const totalQuantity = (items) => getSumOfArr(items.map((item) => item.quantity));

const getSubTotal = (cartData) => getSumOfArr(cartData.items.map((product) => product.total));
export const getStripeTotal = (cartData) => caluclateDiscount(cartData).Total * 100;
